    ...
    for group_idx, para_elements in enumerate(textbox_groups):
        ...
        for p_elem in para_elements:
            try:
                ...
                #     group_lines.append(img_html_content)
                need_ocr = []
                not_ocr = []
                for idd,(filename, img_bytes, cx_emu) in enumerate(img_list):
                    rel_path = f"images/{filename}"
                    images[rel_path] = img_bytes
                    if not docx_extract_images:
                        if cx_emu and content_width:
    ...
    for i in range(nrows):
        ...
        for j in range(ncols):
            ...
            for para in cell.paragraphs:
                ...
                #     content_parts.append.append(img_html_content)
                need_ocr = []
                not_ocr = []
                for idd,(filename, img_bytes, cx_emu) in enumerate(img_list):
                    rel_path = f"images/{filename}"
...
def _convert_body(
    ...
    **kwargs
    ...
    field_state = _FieldState()
    total_range = list(_flatten_body(doc.element.body))
    total_range = track(
        total_range, 
        total = len(total_range), 
        description=f"File[{verbose_name}]:",
        disable=not verbose,
        refresh_per_second=1
    )    
    
    for child in total_range:
        ...
        **kwargs
        ...
        try:
            ...
            if docx_extract_headers_footers:
                ...
                md_text = "\n".join(parts)
            md_text = clean_text_linebreak(md_text)
            time_elapse = time.perf_counter() - start_time
            images = images if not docx_extract_images else {}
            res.append({
                "type": "docx",
                "content": md_text,
