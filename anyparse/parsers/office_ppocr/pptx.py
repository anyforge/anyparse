    ...
    def invoke_item(
        ...
        **kwargs
        ...
        try:
            ...
            total_range = list(enumerate(prs.slides))
            total_range = track(
                total_range, 
            ...
            for slide_idx,slide in total_range:
                ...
                for shape in slide.shapes:
                    self._process_shape(
                        ...
                        images = slide_images, 
                        image_counter = slide_image_counter, 
                ...
                for g in groups:
                    g_content = "\n".join(g)
                    slides_md.append(g_content)
                slides_content = "\n\n".join(slides_md)
                slides_content = clean_text_linebreak(slides_content)
                
                time_elapse = time.perf_counter() - start_time
                slide_images = slide_images if not pptx_extract_images else {}
                res.append({
        ...
        **kwargs        
        ...
        if isinstance(shape, Picture):
            try:
                img = shape.image
                image_counter[0] += 1
                filename = f"image_{image_counter[0]}.{img.ext}"
                rel_path = f"images/{filename}"
                img_bytes = img.blob
                images[rel_path] = img_bytes
                if not pptx_extract_images:
        ...
        # 3. Chart
        if shape.has_chart:
            chart_res = self._chart_to_html(
                chart = shape.chart,
                **kwargs     
            )
            slide_parts.append(chart_res)
            return

        # 4. Table
        if shape.has_table:
            table_res = self._table_to_html(
                table = shape.table, 
                slide_part = slide_part, 
                image_counter_list = image_counter, 
                images = images,
                pptx_extract_images = pptx_extract_images,
                pdflconverter = pdflconverter,
                **kwargs     
            )
            slide_parts.append(table_res)
            return
            ...
            for paragraph in shape.text_frame.paragraphs:
                ...
                if placeholder_type in [
                    ...
                    if placeholder_type == PP_PLACEHOLDER.SUBTITLE:
                        title_level = 3
                    slide_parts.append(f"{'#'*title_level} {text}")
                else:
        ...
        **kwargs
        ...
        try:
            ...
            if has_date_ax and categories:
                ...
                for c in categories:
                    try:
                        dt = datetime.datetime(1899, 12, 30) + datetime.timedelta(days=float(c))
        ...
        **kwargs
        ...
        for i, row in enumerate(table.rows):
            ...
            for j, cell in enumerate(row.cells):
                ...
                #             filename = f"image_{image_counter_list[0]}.{ext}"
                #             rel_path = f"images/{filename}"
                #             img_bytes = image_part.blob
                #             images[rel_path] = img_bytes
                #             if not pptx_extract_images:
                #                 img_html_content = f'<img src="images/{filename}" width="100%">'
                #             else:
                #                 rawimage = Image.open(BytesIO(img_bytes))
                #                 image_res = pdflconverter.invoke_image(
                #                     file = rawimage,
                ...
                #                     if image_item and isinstance(image_item[0], dict):
                #                         img_html_content = image_item[0].get('content','')
                #             content_parts.append(img_html_content)                              
                #         except:
                #             pass
                need_ocr = []
                not_ocr = []
                for idd,blip in enumerate(blips):
                    r_embed = blip.get(f"{_REL_NS}embed")
                    if r_embed:
                        try:
                            image_part = slide_part.related_parts[r_embed]
