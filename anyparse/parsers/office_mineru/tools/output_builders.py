...
import re
from .office_enum_class import MakeMode, BlockType, ContentType, ContentTypeV2
...
def make_blocks_to_content_list(para_block, img_buket_path, page_idx):
    ...
    if para_type in [
        ...
        for span in _iter_body_spans(
            ...
            if span.get('image_base64', ''):
                para_content['img_path'] = f"{span['image_base64']}"
    ...
    def get_data_from_spans(lines):
        ...
        for span in _iter_block_spans(synthetic_block):
            ...
            if span_type == ContentType.TABLE:
                return span.get('image_base64', ''), span.get('html', '')
            elif span_type == ContentType.CHART:
                return span.get('image_base64', ''), span.get('content', '')
            elif span_type == ContentType.IMAGE:
                return span.get('image_base64', ''), ''
