...
from .tools.office_enum_class import BlockType,MakeMode
...
from .tools.office_chart import extract_chart_html_from_ooxml
from .tools.office_stream import read_stream_bytes_from_start, rewind_stream
from .tools.pptx_package_normalizer import normalize_pptx_package
from .tools.xycut_pp_sorter import sort_entries
from .tools.office_rich_text import (
...
class PptxConverter(BaseConverter):
    ...
    def invoke_item(
        ...
        **kwargs
        ...
        try:
            ...
            image_dir = './'
            for slide_idx,(block,page_info) in enumerate(zip(results,pdf_info)):
                page_time = self.pages_timings.get(slide_idx, 0.0)
    ...
    def _walk_linear(self, pptx_obj: presentation.Presentation):
        ...
        # 遍历每一张幻灯片
        total_slides = pptx_obj.slides
        with Progress() as progress:
            # 添加一个任务，total 为幻灯片总数，初始状态根据 verbose 决定是否禁用
            task = progress.add_task(
                f"File[{self.file.name}]:", 
                total=len(total_slides), 
                disable=not self.verbose
            ) 
        
            for slide_idx, slide in enumerate(total_slides):
                ...
                self.pages_timings[slide_idx] = time_elapse
