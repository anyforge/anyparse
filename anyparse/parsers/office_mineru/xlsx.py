...
from typing import BinaryIO, Annotated, cast, Optional
...
from .tools.office_enum_class import BlockType,MakeMode, ContentType
from .tools.office_image import (
    is_vector_image,
    serialize_vector_image_with_placeholder,
)
from .tools.utils import image_to_b64str
...
from .tools.office_stream import read_stream_bytes_from_start, rewind_stream
from .tools.xlsx_package_normalizer import normalize_xlsx_package
from .tools.model_output_to_middle_json import result_to_middle_json
from .tools.output_builders import union_make as office_union_make
...
class XlsxConverter(BaseConverter):
    def __init__(
        ...
        self.equation_bookends_start: str = "<eq>"
        self.equation_bookends_end: str = "</eq>"
        self.equation_bookends: str = self.equation_bookends_start+"{EQ}"+self.equation_bookends_end  # 公式标记格式
    ...
    def invoke_item(
        ...
        **kwargs
        ...
        try:
            ...
            image_dir = './'
            for sheet_idx,(block,page_info) in enumerate(zip(results,pdf_info)):
                if not block:
                    continue
                sheet_name = ""
                if block[0].get("type","") == BlockType.TITLE:
                    sheet_name = block[0].get("content", "")
                    if isinstance(sheet_name, list):
                        content_item = [f"{x.get('content','')}" for x in sheet_name]
                        content_item = set(content_item)
                        sheet_name = ''.join(content_item)                    
                    
                page_time = self.pages_timings.get(sheet_idx, 0.0)
                start_time = time.perf_counter()
                page_content = office_union_make([page_info], f_make_md_mode, image_dir)
                layout_block = []
                for order_idx, item in enumerate(block):
                    item["order_id"] = order_idx
                    layout_block.append(item)
                if excel_extract_images:
                    raw_need_ocr = {}
                    for item in layout_block:
                        if item["type"] == BlockType.IMAGE:
                            ...
                            if not image_base64:
                                continue
                            k = f"![]({image_base64})"
                            raw_need_ocr[k] = image_base64
                    need_ocr = []
                    need_ocr_list = []
                    for idx, (k,v) in enumerate(raw_need_ocr.items()):
                        need_ocr.append([idx, k])
                        need_ocr_list.append(v)
                    if need_ocr:
                        image_res = pdflconverter.invoke_image(
                            file = need_ocr_list,
                            **kwargs
                        )    
                        for (idx, image_name), image_item in zip(need_ocr, image_res): 
                            img_html_content = ''      
                            if image_item and isinstance(image_item[0], dict):
                                img_html_content = image_item[0].get('content','') 
                            page_content = page_content.replace(image_name, img_html_content)
                
                ### 公式
                # page_formula = self.pages_formula_map.get(sheet_idx, [])
                # if page_formula:
                #     exist_layout_num = len(layout_block) + 1
                #     for fd,formula in enumerate(page_formula):
                #         order_idx = exist_layout_num + fd
                #         layout_block.append({
                #             "type": BlockType.TEXT,
                #             "order_id": order_idx,
                #             "content": formula
    ...
    def _convert_package_stream(self, file_stream: BinaryIO) -> None:
        ...
        try:
            ...
            if self.workbook is not None:
                ...
                total_sheets = [1 for sheet in self.workbook.worksheets if not (not self.include_hidden_sheets and sheet.sheet_state != Worksheet.SHEETSTATE_VISIBLE)]
                with Progress() as progress:
                    # 添加一个任务，total 为幻灯片总数，初始状态根据 verbose 决定是否禁用
                    task = progress.add_task(
                        f"File[{self.file.name}]:", 
                        total=len(total_sheets), 
                    ...
                    for idx, sheet in enumerate(self._iter_sheets_to_convert(), start=1):
                        ...
                        # drawing formula
                        latexs = _extract_drawing_math(self.zf, idx - 1)
                        if latexs:
                            self.pages_formula_map[idx-1] = []
                            for latex in latexs:
                                formula = self.equation_bookends.format(EQ=latex)
                                self.pages_formula_map[idx-1].append(formula)
                                sheet_pages[-1][-1].append({
                                    "type": BlockType.TEXT,
                                    "content": formula
                                })
                                
                        self.pages_timings[idx-1] = time.perf_counter() - start_time
