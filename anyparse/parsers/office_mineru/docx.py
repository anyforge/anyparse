...
from .tools.docx_package_normalizer import normalize_docx_package
    ...
    def invoke_item(
        ...
        **kwargs
        ...
        try:
            ...
            image_dir = './'      
            for body_idx,(block,page_info) in enumerate(zip(results,pdf_info)):
                start_time = time.perf_counter()
                ...
                if docx_extract_headers_footers:
                    ### 处理页眉页脚
                    headers = []
                    footers = []
                    for item in layout_block:
                        ...
                        if isinstance(content, list):
                            content_item = [f"{x.get('content','')}" for x in content]
                            content_item = set(content_item)
                            content = ''.join(content_item)
                        if item["type"] == BlockType.HEADER:
                            headers.append(content)
                        elif item["type"] == BlockType.FOOTER:
                    ...
                    if headers:
                        ...
                        for x in set(headers):
                            line = f" <p>{x}</p>"
                            seen_headers.append(line)
                        seen_headers = "\n".join(seen_headers)
                        headers_content = f"<header>\n{seen_headers}\n</header>"
                        page_content = f"{headers_content}\n\n---\n\n{page_content}"

                    if footers:
                        seen_footers = []
                        for x in set(footers):
                            line = f" <p>{x}</p>"
                            seen_footers.append(line)
                        seen_footers = "\n".join(seen_footers)
                        footers_content = f"<footer>\n{seen_footers}\n</footer>"
